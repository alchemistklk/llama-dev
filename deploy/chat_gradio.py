...
with gr.Blocks() as demo:
    ...
    def bot(history, temperature, top_p, slider_context_times):
        if not pandas.isnull(history[-1][-1]):
            history[-1][1] = None
            yield history
        ...
        prompt += (
                "<s>Human: " + history[-1][0].replace("<br>", "\n") + "\n</s>Assistant:"
        )
...
if __name__ == "__main__":
    ...
    if torch.__version__ >= "2" and sys.platform != "linux":
